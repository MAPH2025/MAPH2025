...
double hash_time;
double lock_time;
double target_1_time;
double target_2_time;
double target_3_time;
double target_4_time;
 ...
 public:
  ...
  Value_t Get(T key, bool flag) { 
    ...
    // {
      ...
      tmp = Get(key);
    // }
    // std::cout << "PM Collect: " <<   (media_rd - media_wr) * 1024 * 1024 / 4 / 64 << std::endl;
    return tmp; }
  void Recovery() {
    if (resizing) {
  ...
  void getNumber() {
    ...
    std::cout << "search in target 1 time: " << target_1_time / CLOCKS_PER_SEC << std::endl;
    std::cout << "search in target 2 time: " << target_2_time / CLOCKS_PER_SEC << std::endl;
    std::cout << "search in target 3 time: " << target_3_time / CLOCKS_PER_SEC << std::endl;
    std::cout << "search in target 4 time: " << target_4_time / CLOCKS_PER_SEC << std::endl;
    std::cout << "search in target 1 count: " << target_1_cnt << std::endl;
    std::cout << "search in target 2 count: " << target_2_cnt << std::endl;
    std::cout << "search in target 3 count: " << target_3_cnt << std::endl;
    std::cout << "search in target 4 count: " << target_4_cnt << std::endl;
    std::cout << "lock count: " << lock_cnt << std::endl;
...
void LevelHashing<T>::resize(PMEMobjpool *pop) {
  ...
  << (double)(level_item_num[0] + level_item_num[1]) / (addr_capacity * ASSOC_NUM * 3 / 2)
  << std::endl;
  resizing = true;
  ...
  for (i = 0; i < 2; i++) {
    {
        ...
        if constexpr (std::is_pointer_v<T>) {
          ...
          if (buckets[i][f_idx].token[j] == 1 &&
            ...
            if(i == 0)
            {
              target_1_cnt ++;
              gettimeofday(&stop, NULL);
              target_1_time += (double)(stop.tv_usec - start.tv_usec) + 
                  (stop.tv_sec - start.tv_sec) * 1000000;
            }else
            {
              target_3_cnt ++;
              gettimeofday(&stop, NULL);
              target_3_time += (double)(stop.tv_usec - start.tv_usec) + 
                  (stop.tv_sec - start.tv_sec) * 1000000;
            }
            #endif
            return buckets[i][f_idx].slot[j].value;
      ...
      {
        gettimeofday(&stop, NULL);
        target_2_time += (double)(stop.tv_usec - start.tv_usec) + 
            (stop.tv_sec - start.tv_sec) * 1000000;
      }else
      {
        gettimeofday(&stop, NULL);
        target_4_time += (double)(stop.tv_usec - start.tv_usec) + 
            (stop.tv_sec - start.tv_sec) * 1000000;
      }
      gettimeofday(&start, NULL);
      #endif
