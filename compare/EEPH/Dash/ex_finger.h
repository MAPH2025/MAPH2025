...
/* read time */
double read_table_time;
double hash_time;
double search_in_stash_time;
double read_target_time;
double read_neighbor_time;
double search_in_neighbor_time;
double search_in_target_time;

/* delete time */
double dread_table_time;
double dhash_time;
double dsearch_time;
double dtarget_time;
double dneighbor_time;
double dverify_time;

/* insert time */
double ihash_time;
double i_get_table_time;
double i_direct_time;
double split_time;
double extend_time;

double init_time;
double double_time;
double read_test_time;
/* debug count */
int target_cnt;
int neighbor_cnt;
int stash_cnt;
...
struct Bucket {
...
#ifdef PMEM
    if (nt_flush) {
      PMAllocator::NTWrite32(reinterpret_cast<uint32_t *>(&bitmap), new_bitmap);
    } else {
      bitmap = new_bitmap;
    }
#else
  ...
  int Insert(T key, Value_t value, uint8_t meta_hash, bool probe) {
    ...
    _[slot].key = key;
#ifdef PMEM
    PMAllocator::Persist(&_[slot], sizeof(_[slot]));
#endif
...
struct Directory {
  ...
  static void New(PMEMoid *dir, size_t capacity, size_t version) {
    ...
    PMAllocator::Allocate(dir, kCacheLineSize,
                        sizeof(Directory<T>) + sizeof(table_p) * capacity,
                        callback, reinterpret_cast<void *>(&callback_args));
    
#else
    PMAllocator::Allocate((void **)dir, kCacheLineSize, sizeof(Directory<T>));
    new (*dir) Directory(capacity, version, tables);
#endif
  }

#ifdef DASH_HYBRID
  static void NewHybrid(Directory<T> **dir, size_t capacity, size_t version)
  {
    *dir = (Directory<T> *)malloc(sizeof(Directory<T>) + sizeof(table_p) * capacity);
    Directory<T> *dir_ptr = *dir;
    dir_ptr->version = version;
    dir_ptr->global_depth = static_cast<size_t>(log2(capacity));
  }
#endif
};
...
struct TlsTablePool {
  ...
  static void AllocateMore() {
    ...
    PMAllocator::Allocate(&p_all_tables, kCacheLineSize,
...
struct Table {
  static void New(PMEMoid *tbl, size_t depth, PMEMoid pp) {
    ...
    *tbl = pmemobj_oid(ptr);
    ...
    PMAllocator::Allocate(tbl, kCacheLineSize, sizeof(Table<T>), callback,
                        reinterpret_cast<void *>(&callback_para));
#endif
#else
    PMAllocator::ZAllocate((void **)tbl, kCacheLineSize, sizeof(Table<T>));
...
#ifdef PMEM
      PMAllocator::Persist(&next_neighbor->bitmap, sizeof(next_neighbor->bitmap));
#endif
      neighbor->unset_hash(displace_index);
      neighbor->Insert_displace(key, value, meta_hash, displace_index, true);
      neighbor->release_lock();
#ifdef PMEM
      PMAllocator::Persist(&neighbor->bitmap, sizeof(neighbor->bitmap));
#endif
      target->release_lock();
#ifdef COUNTING
...
#ifdef PMEM
      PMAllocator::Persist(&prev_neighbor->bitmap, sizeof(prev_neighbor->bitmap));
#endif
      target->unset_hash(displace_index);
      target->Insert_displace(key, value, meta_hash, displace_index, false);
      target->release_lock();
#ifdef PMEM
      PMAllocator::Persist(&target->bitmap, sizeof(target->bitmap));
#endif
      neighbor->release_lock();
#ifdef COUNTING
      __sync_fetch_and_add(&number, 1);
...
#ifdef PMEM
        PMAllocator::Persist(&curr_bucket->bitmap, sizeof(curr_bucket->bitmap));
#endif
...
void Table<T>::HelpSplit(Table<T> *next_table) {
  ...
  PMAllocator::Persist(&next_table->pattern, sizeof(next_table->pattern));
  pattern = old_pattern;
  PMAllocator::Persist(&pattern, sizeof(pattern));

#ifdef PMEM
  PMAllocator::Persist(next_table, sizeof(Table));
  ...
  PMAllocator::Persist(this, sizeof(Table));
#endif
}
...
Table<T> *Table<T>::Split(size_t _key_hash) {
  ...
  PMAllocator::Persist(&state, sizeof(state));
  Table<T>::New(&next, local_depth + 1, next);
  Table<T> *next_table = reinterpret_cast<Table<T> *>(pmemobj_direct(next));

  next_table->state = -2;
  PMAllocator::Persist(&next_table->state, sizeof(next_table->state));
  next_table->bucket
...
class Finger_EH : public Hash<T> {
 public:
  ...
  void ShutDown() {
    clean = true;
    PMAllocator::Persist(&clean, sizeof(clean));
  }

  int count_records(int *seg_count)
  {
    size_t _count = 0;
    Directory<T> *seg = dir;
  ...
  void getNumber() {
    ...
    std::cout << "read table time: " << (double)read_table_time / CLOCKS_PER_SEC << std::endl;
    std::cout << "read target time: " << (double)read_target_time / CLOCKS_PER_SEC << std::endl;
    std::cout << "read neighbor time: " << (double)read_neighbor_time / CLOCKS_PER_SEC << std::endl;
    std::cout << "search in target time: " << (double)search_in_target_time / CLOCKS_PER_SEC << std::endl;
    std::cout << "search in neighbor time: " << (double)search_in_neighbor_time / CLOCKS_PER_SEC << std::endl;
    std::cout << "search in stash time: " << (double)search_in_stash_time / CLOCKS_PER_SEC << std::endl;
    std::cout << "================== Search Time ==========================" << std::endl;

    std::cout << "================== Delete Time ==========================" << std::endl;
    std::cout << "hash time: " << (double)dhash_time / CLOCKS_PER_SEC << std::endl;
    std::cout << "read table time: " << (double)dread_table_time / CLOCKS_PER_SEC << std::endl;
    std::cout << "target time: " << (double)dtarget_time / CLOCKS_PER_SEC << std::endl;
    std::cout << "neighbor time: " << (double)dneighbor_time / CLOCKS_PER_SEC << std::endl;
    std::cout << "verify time: " << (double)dverify_time / CLOCKS_PER_SEC << std::endl;
    ...
    std::cout << "get table time: " << (double)i_get_table_time / CLOCKS_PER_SEC << std::endl;
    std::cout << "insert directly time: " << (double)i_direct_time / CLOCKS_PER_SEC << std::endl;
    std::cout << "split time: " << (double)split_time / CLOCKS_PER_SEC << std::endl;
    std::cout << "extend time: " << (double)extend_time / CLOCKS_PER_SEC << std::endl;
    std::cout << "init time: " << (double)init_time / CLOCKS_PER_SEC << std::endl;
    std::cout << "double time: " << (double)double_time / CLOCKS_PER_SEC << std::endl;
    std::cout << "================== Insert Time ==========================" << std::endl;

    std::cout << "read test time: " << (double)read_test_time / CLOCKS_PER_SEC<< std::endl;

    std::cout << "==================    Count    ==========================" << std::endl;
    std::cout << "cnt: " << cnt << std::endl;
    std::cout << "split cnt: " << split_cnt << std::endl;
    std::cout << "split ins cnt: " << split_ins_cnt << std::endl;
    std::cout << "double cnt: " << double_cnt << std::endl;
    std::cout << "search cnt: " << search_cnt << std::endl;
    std::cout << "delete cnt: " << delete_cnt << std::endl;
    std::cout << "==================    Count    ==========================" << std::endl;
  }
...
Finger_EH<T>::Finger_EH(size_t initCap, PMEMobjpool *_pool) {
  ...
  pool_addr = _pool;
#ifdef DASH_HYBRID
  Directory<T>::NewHybrid(&dir, initCap, 0);
#else
  ...
  { 
    int index = rand() % initCap;
    int b_index = rand() % 14;
    Table<T> *table__ = dir->_[index];
    if(table__->bucket[b_index].bitmap != 0)
      printf("dash: errorrrrrrrrrrr\n");
  }
  gettimeofday(&stop, NULL);
      read_test_time += (double)(stop.tv_usec - start.tv_usec) + 
...
void Finger_EH<T>::Halve_Directory() {
  ...
  TX_BEGIN(pool_addr) {
    ...
    PMAllocator::Free(reserve_item, dir);
    dir = new_dir;
    back_dir = OID_NULL;
  }
  TX_ONABORT {
...
void Finger_EH<T>::Directory_Doubling(int x, Table<T> *new_b, Table<T> *old_b) {
  ...
  // print load factory
  int seg_count = 0;
  int items = count_records(&seg_count);
  std::cout << "load factory: " << (double)items /  (seg_count * kNumPairPerBucket * (kNumBucket + 2)) << std::endl;

  auto capacity = pow(2, global_depth);
#ifdef DASH_HYBRID
  Directory<T> *new_sa;
  Directory<T>::NewHybrid(&new_sa, 2 * capacity, dir->version + 1);
#else
...
#if (defined PMEM) && (! defined DASH_HYBRID)
  PMAllocator::Persist(new_sa,
...
void Finger_EH<T>::Directory_Merge_Update(Directory<T> *_sa, uint64_t key_hash,
  ...
  for (int i = right; i < right + chunk_size / 2; ++i) {
    ...
    PMAllocator::Persist(&dir_entry[i], sizeof(uint64_t));
  }
  ...
  if (target->state != 0) {
    ...
    if (target->state == -2) {
      if (next_table->state == -3) {
        ...
        next_table->state = 0;
        PMAllocator::Persist(&next_table->state, sizeof(int));
      }
    } else if (target->state == -1) {
      if (next_table->pattern == ((target->pattern << 1) + 1)) {
        target->Merge(next_table, true);
        PMAllocator::Persist(target, sizeof(Table<T>));
        target->next = next_table->next;
        PMAllocator::Free(next_table);
      }
    }
    target->state = 0;
    PMAllocator::Persist(&target->state, sizeof(int));
  }
...
void Finger_EH<T>::Recovery() {
  ...
  if (crash_version == 0) {
    ...
    PMAllocator::Persist(dir_entry, sizeof(uint64_t) * length);
  }
}
...
RETRY:
  ...
  #ifdef INSERT_CLOCK
  gettimeofday(&stop, NULL);
  i_get_table_time += (double)(stop.tv_usec - start.tv_usec) +
                (double)(stop.tv_sec - start.tv_sec) * 1000000;
  #endif
  ...
  REINSERT:
    ...
    if (target->local_depth < old_sa->global_depth) {
      ...
      #ifdef INSERT_CLOCK
      gettimeofday(&stop, NULL);
      split_time += (double)(stop.tv_usec - start.tv_usec) +
                (double)(stop.tv_sec - start.tv_sec) * 1000000;
      #endif
    } else {
      Lock_Directory();
    ...
    /*release the lock for the target bucket and the new bucket*/
    new_b->state = 0;
    PMAllocator::Persist(&new_b->state, sizeof(int));
    target->state = 0;
    PMAllocator::Persist(&target->state, sizeof(int));
  ...
  #ifdef INSERT_CLOCK
  gettimeofday(&stop, NULL);
  i_direct_time += (double)(stop.tv_usec - start.tv_usec) +
                (double)(stop.tv_sec - start.tv_sec) * 1000000;
  #endif
  return 0;
}
...
RETRY:
  ...
  gettimeofday(&stop, NULL);
  target_cnt ++;
  read_table_time +=(double)(stop.tv_usec - start.tv_usec) +
                    (double) (stop.tv_sec - start.tv_sec) * 1000000;
  gettimeofday(&start, NULL);
  #endif
  auto old_entry = dir_entry[x];// get xth segment
  ...
  search_cnt ++;
  gettimeofday(&stop, NULL);
  read_target_time += (double)(stop.tv_usec - start.tv_usec) +
            (double) (stop.tv_sec - start.tv_sec) * 1000000;
  ...
  #ifdef SEARCH_CLOCK
  gettimeofday(&stop, NULL);
  read_neighbor_time += (double)(stop.tv_usec - start.tv_usec) +
            (double) (stop.tv_sec - start.tv_sec) * 1000000;
        ...
        for (int i = 0; i < 4; ++i) {
          if (CHECK_BIT(mask, i) &&
            ...
            if (ret != NONE) {
              ...
              #ifdef SEARCH_CLOCK
              gettimeofday(&stop, NULL);
              search_in_stash_time += (double)(stop.tv_usec - start.tv_usec) +
                        (double) (stop.tv_sec - start.tv_sec) * 1000000;
              #endif
              return ret;
            }
  ...
  do {
    ...
    if (left_seg->Acquire_and_verify(_pattern0)) {
      if (right_seg->Acquire_and_verify(_pattern1)) {
        ...
        PMAllocator::Persist(&left_seg->local_depth, sizeof(uint64_t));
        left_seg->state = -1;
        PMAllocator::Persist(&left_seg->state, sizeof(int));
        right_seg->state = -1;
        PMAllocator::Persist(&right_seg->state, sizeof(int));
      REINSERT:
        old_dir = dir;
        ...
        TX_BEGIN(pool_addr) {
          ...
          PMAllocator::Free(reserve_item, right_seg);
        ...
        PMAllocator::Persist(&left_seg->pattern, sizeof(uint64_t));
        left_seg->state = 0;
        PMAllocator::Persist(&left_seg->state, sizeof(int));
...
bool Finger_EH<T>::Delete(T key) {
  ...
  #ifdef DELETE_CLOCK
  gettimeofday(&stop, NULL);
  dhash_time += (double)(stop.tv_usec - start.tv_usec)
                +(double)(stop.tv_sec - start.tv_sec) * 1000000;
  gettimeofday(&start, NULL);
  #endif
RETRY:
  ...
  #ifdef DELETE_CLOCK
  gettimeofday(&stop, NULL);
  dread_table_time += (double)(stop.tv_usec - start.tv_usec)
                      +(double)(stop.tv_sec - start.tv_sec) * 1000000;
  gettimeofday(&start, NULL);
  #endif
  // uint32_t old_version = 
  //     __atomic_load_n(&target->version_lock, __ATOMIC_ACQUIRE);
  // uint32_t old_neighbor_version = 
  //     __atomic_load_n(&neighbor->version_lock, __ATOMIC_ACQUIRE);
  target->get_lock();
  if (!neighbor->try_get_lock()) {
    target->release_lock();
    goto RETRY;
  ...
  delete_cnt ++;
  gettimeofday(&stop, NULL);
  dtarget_time += (double)(stop.tv_usec - start.tv_usec)
                +(double)(stop.tv_sec - start.tv_sec) * 1000000;
  gettimeofday(&start, NULL);
  #endif
  old_sa = dir;
  ...
  if (ret == 0) {
    ...
    #ifdef DELETE_CLOCK
    gettimeofday(&stop, NULL);
    dverify_time += (double)(stop.tv_usec - start.tv_usec)
                    +(double)(stop.tv_sec - start.tv_sec) * 1000000;
    gettimeofday(&start, NULL);
    #endif
    return true;
  ...
  if (ret == 0) {
    ...
    #ifdef DELETE_CLOCK
    gettimeofday(&stop, NULL);
    dneighbor_time += (double)(stop.tv_usec - start.tv_usec) +
              (double) (stop.tv_sec - start.tv_sec) * 1000000;
    #endif
    return true;
  }
    ...
    if (test_stash == true) {
      ...
      for (int i = 0; i < stashBucket; ++i) {
        ...
        if (ret == 0) {
          ...
          stash->release_lock();
#ifdef PMEM
          PMAllocator::Persist(&curr_stash->bitmap, sizeof(curr_stash->bitmap));
#endif
          auto bucket_ix = BUCKET_INDEX(key_hash);
