...
// std::string pool_name = "/mnt/pmem0/cq-hash/";
// DEFINE_string(index, "eeph",
...
// DEFINE_uint64(i, 131072, "the initial number of segments in extendible hashing");
...
//   if (index_type == "eeph") {
//     std::cout << "Initialize EEPH" << std::endl;
//     std::string index_pool_name = pool_name + "pmem_eeph.data";
//     if(FileExists(index_pool_name.c_str())) file_exist = true;
//     // open the pool
//     PMAllocator::Initialize(index_pool_name.c_str(), pool_size);
//     // get hash object
...
//       uint32_t bucket_number = 3000000;
//       uint32_t cell_number;
//       cell_number = 6000000;
//       uint8_t cell_hash = 16;
      
//       std::cout << "creat EEPH object" << std::endl;
//       new (eh) eeph::EEPH<T>(bucket_number, cell_number, cell_hash);
//     }else
//     {
//       std::cout << "get eeph object" << std::endl;
//       new (eh) eeph::EEPH<T>();
//     }
//   } else if(index_type == "dash") 
//   {
//     std::cout << "Initialize Dash-EH" << std::endl;
//     std::string index_pool_name = pool_name + "pmem_dash.data";
//     if (FileExists(index_pool_name.c_str())) file_exist = true;
//     PMAllocator::Initialize(index_pool_name.c_str(), pool_size);
// #ifdef PREALLOC
...
//         PMAllocator::GetRoot(sizeof(extendible::Finger_EH<T>)));
//     if (!file_exist) {
//       new (eh) extendible::Finger_EH<T>(seg_num, PMAllocator::Get()->pm_pool_);
//     } else {
...
//     if (!file_exist) {
//       new (eh) cceh::CCEH<T>(seg_num, PMAllocator::Get()->pm_pool_);
//     } else {
//       new (eh) cceh::CCEH<T>();
//     }
...
//   if (persist) {
//     PMAllocator::Persist(memory_region, generate_num * sizeof(uint64_t));
//   }
// }
...
//   if (persist) {
//     PMAllocator::Persist(memory_region,
...
//       if(index->Insert(key_array[i], DEFAULT))
//         count ++;
//       else
//         not_found ++;
//     }
...
//       if(index->Insert(var_key, DEFAULT))
//         count ++;
//       else
//         not_found ++;
//     }
//   }
//   std::cout << "insert successfully number " << count << std::endl;
//   std::cout << "insert failed number " << not_found << std::endl;
//   end_notify(_range);
// }
...
//         if(index->Insert(key_array[j], DEFAULT, true))
//           count ++;
//         else
//           not_found ++;
//       }
...
//         if(index->Insert(key_array[i], DEFAULT, true))
//           count ++;
//         else
//           not_found ++;
//       }
...
//                       &concurr_search);
//       }
//       std::cout << "PM Collect: " <<  imc_rd << " " << imc_wr << " " << media_rd << " " << media_wr << std::endl;
//     #else
//       GeneralBench<T>(rarray, index, thread_num, operation_num, "Pos_search",
//                         &concurr_search);
//     #endif
