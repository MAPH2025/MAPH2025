...
#define BUCKET_CAPACITY 8  // item number in a bucket
...
struct Bucket{
    ...
    static void New(PMEMoid *bkt, uint32_t bucket_number_, size_t arg) {
        auto init_callback = [](PMEMobjpool *pool, void *ptr, void *arg) {
            ...
            pmemobj_persist(pool, bucket_p, sizeof(Bucket<T>) * bucket_number__);
            pmem_memory_use += (sizeof(Bucket<T>) * bucket_number__);
            return 0;
        };
    ...
    bool verify_unique(T key_)
    {
        ...
        int i;
            ...
            mask &= ((uint64_t)1 << BUCKET_CAPACITY) - 1;
        #else
    ...
    */
    ...
    inline void collect_kv(int cell_k, _Pair<T> *kvs, uint8_t local_depth, 
        ...
        {
            uint64_t valid = bitmap & ((uint64_t)1 << i);
            if(valid == 0)
                continue;
...
#ifdef USING_SIMD
        const __m256i item = _mm256_set1_epi32(*(int*)key_);
...
struct Layer
{
    ...
    static void New(PMEMoid *layer, uint32_t gd, size_t arg)
    {
        auto init_callback = [](PMEMobjpool *pool, void *layer_p, void *arg)
        {
            ...
            pmem_memory_use += (sizeof(Layer<T>) + sizeof(bucket_p) * layer_num_);
            return 0;
        };
...
public:
    ...
    void testExpansion(){
        dynamic_extension(layers->global_depth);
    }
    void getPmemNumber(bool set){
        if(set){
            pmem_read_count=0;
            pmem_write_count=0;
        }else{
            std::cout<<"pmem_read_count:"<<pmem_read_count<<std::endl;
            std::cout<<"pmem_write_count:"<<pmem_write_count<<std::endl;
        }
    }
    void getNumber()
    {
        ...
        std::cout << "dram use: "<<(double)dram_memory_use/(1024*1024*1024)<<" pmem use: "<<(double)pmem_memory_use/(1024*1024*1024)<<std::endl;
...
private:
    ...
    */
                        ...
                        uint8_t global_depth, uint32_t initial_cell, uint32_t *max_pos, uint32_t *max_guide)
    {
        ...
        for(int guide = 0; guide < cell_hash; ++ guide){
            ...
            if(tmp_d == d)
            {
                ...
                {
                    pmem_read_count += 1;
                    uint64_t valid = bucket[d].bitmap & ((uint64_t)1 << i);
                    if(valid == 0)
                        continue;
    ...
    REHASH:    
        ...
        for(guide = 0; guide < cell_hash; ++guide){
            ...
            for(int j = 0; j < BUCKET_CAPACITY; ++j){
                pmem_read_count += 1;
                uint64_t valid = bucket[d].bitmap & ((uint64_t)1 << j);
                if(valid == 0)
                    continue;
                                ...
                                uint32_t initial_cell, Bucket<T> *source_bucket, Bucket<T> *target_bucket, T key, Value_t value)
    {
        ...
        {
            pmem_read_count += 1;
            uint64_t valid = source_bucket->bitmap & ((uint64_t)1 << i);
            if(valid == 0)
                continue;
            ...
            {
                // set valid bitmap
                source_bucket->bitmap &= ~((uint64_t)1 << i);
                source_bucket->used --;
                bucket_items --;
...
public:
    ...
    REINSERT:
        ...
        //printf("GOES HERE 1, result:%d\n",result);
        if(result == 1)
        {
            pmem_write_count += 1;
            // if local depth does not consist, reinsert.
            uint8_t new_local_depth = cell[k].local_depth;
            //printf("GOES HERE 2, k:%d, new_local_depth:%d, local_depth:%d\n",k,new_local_depth,local_depth);
            if(new_local_depth != local_depth)
                goto REINSERT;
            insert_dire_cnt ++;
            #ifdef INSERT_CLOCK
...
QUERY:
        ...
        //simulate update
        ret = bucket[d].query_key_in_bucket(key, value);
        bucket[d].release_lock();
        //------------
        #ifdef SEARCH_CLOCK
            gettimeofday(&stop, NULL);
...
bool EEPH<T>::find_kv(T key)
{
    ...
    for(int i = 0; i < layer_number; i++)
    {
        ...
        for(int j = 0; j < init_bucket_number; j++)
        {
            ...
            {
                uint64_t valid = bucket.bitmap & ((uint64_t)1 << k);
...
void EEPH<T>::recovery()
{
    ...
    // if(find_kv((T)8517097267634966620))
    //     printf("herrrrrrr\n");
    printf("REACH HERE,layers:%p\n",layers);
    uint8_t global_depth = layers->global_depth;
    printf("REACH HERE2,%p\n",layers->bp[0]);
    ...
    for(int i = 0; i < cell_number_; i++)
    {
        ...
        for(int j = 0; j < cell_hash; j++)
        {
            ...
            for(int k = 0; k < BUCKET_CAPACITY; k ++)
            {
                ...
                //     printf("herrrrr\n");
                uint64_t valid = bucket_.bitmap & (1 << k);
...
EEPH<T>::EEPH(void){
    ...
    layers->bp[0] = (Bucket<T>*)pmemobj_direct(bp_oid);
    std::cout << "重新初始化layers为: " << layers << std::endl;
    recovery();
};
...
EEPH<T>::EEPH(uint32_t bucket_number_, uint32_t cell_number_, uint8_t cell_hash_){
    ...
    layers = (Layer<T> *)pmemobj_direct(layers_);
    printf("Initial layers addr : %p\n",layers);
    Bucket<T>::New(&bp_oid, bucket_number_, 0);
    layers->bp[0] = (Bucket<T> *)pmemobj_direct(bp_oid);
    initialize_hash_functions();
    #ifdef DIRECTORY
    ...
    #endif
    // #endif
    dram_memory_use += (sizeof(Cell) * cell_number_ + sizeof(EEPH<T>));
