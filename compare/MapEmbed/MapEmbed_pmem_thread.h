...
private:
    ...
    bool insert_kv_to_bucket(const KV_entry& kv, int d, int layer, int k){
        ...
        bucket[d].used++;
        #ifdef PMEM
        pmem_persist(&bucket[d],sizeof(Bucket));
        #endif
        return true;
    }
            ...
            for(int j = 0; j < bucket[d].used; ++j){
                if(bucket[d].layer[j] == layer && bucket[d].k[j] == k){
                    ...
                    if(j == used - 1) {
                        ...
                        bucket[d].used--;
                        #ifdef PMEM
                        pmem_persist(&bucket[d].used, sizeof(int));
                        #endif
...
public:
    MapEmbed(int layer_, int bucket_number_, int* cell_number_, int cell_bit_, int thread_num_){
        ...
        #ifdef PMEM
        // 计算需要的持久内存大小
        size_t size = bucket_number * sizeof(Bucket);
        
        // 创建持久内存文件目录（如果不存在）
        char path[256];
        sprintf(path, "%s_%d", PMEM_PATH, this->bucket_number);

        if ((pmem_addr = pmem_map_file(path, size, PMEM_FILE_CREATE, 0666, &mapped_len, &is_pmem)) == NULL) {
            perror("pmem_map_file");
            exit(1);
        }
        if(is_pmem) printf("REAL PMEM\n");
        bucket = (Bucket*)pmem_addr;
        
        
        pmem_memset_persist(bucket, 0, bucket_number * sizeof(Bucket));
        for (int i = 0; i < bucket_number; ++i) {
            bool check_value = true;
            int used_value = 0;
            pmem_memcpy_persist(&bucket[i].check, &check_value, sizeof(bool));
            pmem_memcpy_persist(&bucket[i].used, &used_value, sizeof(int));
        }
        #else
    ...
    bool query(const char *key, char* value = NULL){
        for(int layer = 0; layer < cell_layer; ++layer){
            ...
            cell_unlock(layer, k, false);
            bucket_lock(d,false);
            query_key_in_bucket(key, d, value);
	...
	bool deletion(const char *key){
        for(int layer = 0; layer < cell_layer; ++layer){
            ...
            for(int i = 0; i < used; ++i){
                if(memcmp(bucket[d].key[i], key, KEY_LEN*sizeof(char)) == 0){
                    memset(bucket[d].value[i], 999, VAL_LEN*sizeof(char));
                    #ifdef PMEM
                    pmem_persist(&bucket[d],sizeof(Bucket));
                    #endif
                    bucket_unlock(d, true);
    ...
    /* Update an entry */
	bool update(const KV_entry& entry){
        for(int layer = 0; layer < cell_layer; ++layer){
            ...
            int used = bucket[d].used;
            for(int i = 0; i < used; ++i){
                if(memcmp(bucket[d].key[i], entry.key, KEY_LEN*sizeof(char)) == 0){
                    memcpy(bucket[d].value[i], entry.value, VAL_LEN*sizeof(char));
                    #ifdef PMEM
                    pmem_persist(bucket[d].value[i],VAL_LEN*sizeof(char));
                    #endif
                    break;
                }
            }
    ...
    void extend(bool lazy = true){  //note! initial bucket number must be multiple of `cell_hash'
        // 修改: 使用PMEM申请新的内存
        #ifdef PMEM
        size_t new_size = 2 * bucket_number * sizeof(Bucket);
        void* new_pmem_addr;
        ...
        // 创建新的持久内存文件
        char path[256];
        sprintf(path, "%s_%d_extended", PMEM_PATH, this->bucket_number);
        
        if ((new_pmem_addr = pmem_map_file(path, new_size, PMEM_FILE_CREATE, 
                                        0666, &new_mapped_len, &new_is_pmem)) == NULL) {
            perror("pmem_map_file for extension");
            exit(1);
        }
        
        Bucket* bucket_new = (Bucket*)new_pmem_addr;
        ...
        for(int i = bucket_number - 1; i >= 0; --i){
           ...
           // 重置检查标志
            ...
            memcpy(&bucket_new[j1].check, &check_value, sizeof(bool));
            memcpy(&bucket_new[j2].check, &check_value, sizeof(bool));
        }
        pmem_persist(bucket_new,new_size);
        // 释放旧的PMEM映射
        pmem_unmap(pmem_addr, mapped_len);
