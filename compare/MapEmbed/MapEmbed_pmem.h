...
public:
    ...
    void extend(bool lazy = true){  //note! initial bucket number must be multiple of `cell_hash'
        ...
        for(int i = bucket_number - 1; i >= 0; --i){
            ...
            // 修改: 使用PMEM持久化操作复制数据
            pmem_memcpy_persist(&bucket_new[j1], &bucket[i], sizeof(Bucket));
            pmem_memcpy_persist(&bucket_new[j2], &bucket[i], sizeof(Bucket));
           // 重置检查标志
            bool check_value = false;
            pmem_memcpy_persist(&bucket_new[j1].check, &check_value, sizeof(bool));
            pmem_memcpy_persist(&bucket_new[j2].check, &check_value, sizeof(bool));
