...
class hash_api {
 public:
  ...
  hash_api(size_t sz = 16 * 1024 * 1024) {
    ...
    std::error_code ec;
  std::filesystem::remove_all(PM_PATH, ec);
...
public:
    ...
    hash_api() {                       // ← 完全按你原来的写法
        const size_t inital_size = pool_size;   // 沿用你之前的 pool_size 变量
        index_pool_name += "VIPER.data";        // 沿用你之前的 index_pool_name
        if (!std::filesystem::exists(index_pool_name))
            viper_db = viper::Viper<std::string, std::string>::create(
                           index_pool_name, inital_size);
        else
            viper_db = viper::Viper<std::string, std::string>::open(
                           index_pool_name);
    }

    ~hash_api() = default;

    std::string hash_name() { return "Viper<string,string>"; }

    /* 下面四个接口签名保持原样，内部只做类型转换 */
    bool find(size_t key, viper::Viper<std::string,std::string>::Client &c,
              uint64_t *v) {
        std::string val_str;
        bool ok = c.get(std::to_string(key), &val_str);
        if (ok && v) *v = std::stoull(val_str);
        return ok;
    }

    bool insert(size_t key, size_t value_len, char *value,
                viper::Viper<std::string,std::string>::Client &c) {
        return c.put(std::to_string(key),
                     std::string(value, value_len));
    }

    bool update(size_t key, size_t value_len, char *value,
                viper::Viper<std::string,std::string>::Client &c) {
        return insert(key, value_len, value, c);
    }

    bool erase(size_t key,
               viper::Viper<std::string,std::string>::Client &c) {
        return c.remove(std::to_string(key));
    }

    viper::Viper<std::string,std::string>::Client get_client() {
        return viper_db->get_client();
    }

    void wait() {}

    void load_factor(viper::Viper<std::string,std::string>::Client &c) {
        viper_db->print_usage();
    }
};
