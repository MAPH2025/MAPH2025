...
namespace HALO {
constexpr bool HALO_DISABLE_RESIZE = false;
constexpr bool HALO_DISABLE_OVERFLOW = false; // 禁止 bucket->next 扩展

// DRAM overflow bucket 数量
inline std::atomic<size_t> OVERFLOW_BUCKET_CNT{0};

// DRAM allocator 分配的总字节数
inline std::atomic<size_t> DRAM_ALLOCATED_BYTES{0};
...
constexpr size_t MAX_BATCHING_SIZE = 1;
...
constexpr size_t PAGE_SIZE = 16 *1024ULL  * 1024 /* bytes */;
...
constexpr size_t TABLE_NUM = 2;
constexpr size_t CORE_NUM = 32;
struct root;
...
constexpr size_t MAX_BUFFER_PAIR_SIZE = 1;
constexpr size_t MAX_WRITE_BUFFER_SIZE = 256;
constexpr size_t MAX_PAIR = 1;
...
constexpr size_t DEAFULT_SEGMENT_SIZE = 4 * 1024 * 1024;
...
struct CLHT {
    ...
    do {
      ...
      if (Likely(bucket->next == INVALID)) {
        if (Unlikely(empty == NULL)) {
          if constexpr (HALO_DISABLE_OVERFLOW) {
            LOCK_RLS(lock);
            return false; // 容量满且不允许扩展
          }
  ...
  size_t ht_status(int resize_increase, int just_print) {
    if constexpr (HALO_DISABLE_RESIZE) {
      if (!TRYLOCK_ACQ(&status_lock)) TRYLOCK_RLS(status_lock);
      return 0;
    }
  ...
  //     int need_resize =
  //         ((full_ratio > 0 && full_ratio > CLHT_PERC_FULL_DOUBLE) ||
  //         expands_max > CLHT_MAX_EXPANSIONS || resize_increase);
  ...
  //       size_t buckets_before = hashtable->num_buckets;
  ...
  //       * =============================== */
  //       printf(
  //           "[RESIZE] buckets: %zu -> %zu | size: %zu -> %zu (diff %+zd) | "
  //           "time: %.2f us\n",
  //           buckets_before, hashtable->num_buckets,
  ...
  pair<size_t, Bucket *> clht_bucket_create_stats(Segment *h, int *resize) {
    ...
    HALO::OVERFLOW_BUCKET_CNT.fetch_add(
...
class Halo {
 public:
  int do_insert_now_cnt = 0;
  Halo(size_t N) {
    cout << typeid(KEY).name() << " " << typeid(VALUE).name() << endl;
    memset(clhts, 0, TABLE_NUM * sizeof(void *));

    if (filesystem::exists(PM_PATH)) {
      Timer t;
      t.start();
      std::cout<<"recover timer start"<<std::endl;
  ...
  void print_memory_usage() {
    ...
    * ============================================================ */
    ...
    size_t dram_main_buckets = 0;
    size_t dram_overflow_buckets =
        OVERFLOW_BUCKET_CNT.load(std::memory_order_relaxed) *
        sizeof(Bucket);

    size_t dram_metadata = 0;
    ...
    for (size_t i = 0; i < TABLE_NUM; i++) {
      ...
      // 主 bucket 数组
      dram_main_buckets += seg->num_buckets * sizeof(Bucket);

      // Segment + CLHT + DRAM allocator 对象
      dram_metadata += sizeof(Segment);
      dram_metadata += sizeof(CLHT);
      dram_metadata += sizeof(DRAM_MemoryManager);
    }

    // DRAM allocator 实际向 OS 申请的 page
    size_t dram_allocator_pages =
        DRAM_ALLOCATED_BYTES.load(std::memory_order_relaxed);

    // 全局元数据
    size_t dram_global_metadata =
        sizeof(Halo) +
        sizeof(MemoryManagerPool) +
        MAX_PAGE_NUM * sizeof(std::atomic_size_t);

    size_t dram_total =
        dram_main_buckets +
        dram_overflow_buckets ;

    /* ============================================================
    * PMem usage (HALO definition)
    * ============================================================ */

    // HALO 中 PMem footprint = 分配的 PMem page 数 × PAGE_SIZE
    size_t pmem_pages = PM_MemoryManager::PAGE_ID;
    size_t pmem_total = pmem_pages * PAGE_SIZE;

    /* ============================================================
    * Output
    * ============================================================ */

    std::cout << "========== HALO Memory Usage ==========\n"
              << "DRAM total : "
              << dram_total / GB << " GB\n"
              << "PMEM total : "
              << pmem_total / GB << " GB\n"
  ...
  void load_factor() {
    ...
    printf("DRAM Allocated by malloc: %.5f GB.\n",
           float(b_m) / 1024 / 1024 / 1024);
    cout<<"do insert now:"<<do_insert_now_cnt<<endl;
