/**
 * This code was taken and modified from https://github.com/DICL/CCEH, the
 * original authors of CCEH.
 *
 * Orignial License:
 ...
 * Change the file location (CCEH_PMEM_POOL_FILE) above the PMemAllocator
 * definition to a location of your choice.
 */
...
#ifdef CCEH_PERSISTENT
static constexpr char CCEH_PMEM_POOL_FILE[] =
    "/mnt/pmem2/viper/cceh-allocator.file";
class PMemAllocator {
 public:
  ...
  void allocate(PMEMoid* pmem_ptr, size_t size) {
    ...
    int ret =
        pmemobj_alloc(pmem_pool_.handle(), pmem_ptr, size, 0, ctor, nullptr);
    if (ret != 0) {
      throw std::runtime_error{std::string("Could not allocate! ") +
                               std::strerror(errno)};
    }
  }
  ...
  void initialize() {
    ...
    pmem_pool_ = pmem::obj::pool_base::create(
        CCEH_PMEM_POOL_FILE, "", 10ul * (1024l * 1024 * 1024), S_IRWXU);
...
#define internal_cas(entry, expected, updated)                 \
  __atomic_compare_exchange_n(entry, expected, updated, false, \
                              __ATOMIC_ACQUIRE, __ATOMIC_ACQUIRE)

#define ATOMIC_LOAD(addr) __atomic_load_n(addr, __ATOMIC_ACQUIRE)

#define ATOMIC_STORE(addr, value) \
  __atomic_store_n(addr, value, __ATOMIC_RELEASE)

#define requires_fingerprint(KeyType) \
  std::is_same_v<KeyType, std::string> || sizeof(KeyType) > 4

#define IS_BIT_SET(variable, mask) ((variable & mask) != 0)

template <typename KeyType>
inline bool vic(KeyType* key, KeyType* expected, KeyType updated) {
  if constexpr (sizeof(KeyType) == 1)
    return internal_cas((int8_t*)key, (int8_t*)expected, (int8_t)updated);
    ...
    throw std::runtime_error("vic not supported for > 16 bytes!");
}
...
struct KeyValueOffset {
  ...
  KeyValueOffset(const block_size_t block_number, const page_size_t page_number,
                 const data_offset_size_t slot)
      : block_number{block_number},
        page_number{page_number},
        data_offset{slot} {}

  static KeyValueOffset Tombstone() { return KeyValueOffset{}; }

  inline std::tuple<block_size_t, page_size_t, data_offset_size_t> get_offsets()
      const {
    return {block_number, page_number, data_offset};
  }

  inline bool is_tombstone() const { return offset == INVALID; }

  inline bool operator==(const KeyValueOffset& rhs) const {
...
// constexpr size_t kMask = (1 << kSegmentBits) - 1;
// constexpr size_t kShift = kSegmentBits;
// constexpr size_t kSegmentSize = (1 << kSegmentBits) * 32;
// constexpr size_t kNumPairPerCacheLine = 2;
...
struct Segment {
  ...
  void* operator new(size_t size) {
    ...
    if (posix_memalign(&ret, 64, size) != 0)
      throw std::runtime_error("bad memalign");
    return ret;
#endif
  }

  template <typename KeyCheckFn>
  int Insert(const KeyType&, IndexV, size_t, size_t, IndexV* old_entry,
             KeyCheckFn);
...
struct Directory {
  ...
  Directory(size_t _depth) {
    ...
    capacity = pow(2, depth);
#ifdef CCEH_PERSISTENT
    PMemAllocator::get().allocate(&pmem_seg_loc_,
                                  sizeof(Segment<KeyType>*) * capacity);
  ...
  bool Acquire(void) {
    bool unlocked = false;
    return vic(&lock, &unlocked, true);
  }

  bool Release(void) {
    bool locked = true;
    return vic(&lock, &locked, false);
  }
...
class CCEH {
 public:
  ...
  size_t get_directory_capacity() const { return dir->capacity; }
  ...
  size_t get_global_depth() const { return dir->depth; }
...
template <typename KeyType>
template <typename KeyCheckFn>
int Segment<KeyType>::Insert(const KeyType& key, IndexV value, size_t loc,
                             size_t key_hash, IndexV* old_entry,
                             KeyCheckFn key_check_fn) {
  ...
  for (unsigned i = 0; i < kNumPairPerCacheLine * kNumCacheLine; ++i) {
    ...
    if (invalidate && vic(&_[slot].key, &_key, INVALID)) {
      _[slot].value = IndexV::Tombstone();
    }

    if (vic(&_[slot].key, &LOCK, SENTINEL)) {
      ...
      while (!vic(&_[slot].value.offset, &old_value.offset, value.offset)) {
      }
      if (value.is_tombstone()) {
        IndexK expected = key_checker;
        vic(&_[slot].key, &expected, INVALID);
      }
...
IndexV CCEH<KeyType>::Insert(const KeyType& key, IndexV value,
  ...
  if constexpr (std::is_same_v<KeyType, std::string>) {
    key_hash = h(key.data(), key.length());
  } else {
  ...
  while (true) {
    ...
    IndexV old_entry{};
    auto ret =
        target->Insert(key, value, loc, key_hash, &old_entry, key_check_fn);
    ...
    {  // CRITICAL SECTION - directory update
      ...
      if (dir->_[x]->local_depth - 1 < dir->depth) {  // normal split
        ...
        if (depth_diff == 0) {
          ...
          persist((char*)&dir->_[x + chunk_size / 2],
        ...
        if (!vic(&dir, &dir_old, _dir)) {
          throw std::runtime_error(
              "Could not swap dirs. This should never happen!");
        }
...
void CCEH<KeyType>::Remove(IndexV* offset) {
  ...
  vic(&offset->offset, &expected_value, IndexV::Tombstone().offset);
...
IndexV CCEH<KeyType>::Get(const KeyType& key, KeyCheckFn key_check_fn) {
  ...
  while (true) {
    ...
    uint64_t lock = sema.load();
    if ((lock & SPLIT_REQUEST_BIT) != 0 ||
        !sema.compare_exchange_weak(lock, lock + 1)) {
      continue;
    }
...
template <typename KeyType>
size_t CCEH<KeyType>::Capacity(void) const {
  std::unordered_map<Segment<KeyType>*, bool> set;
  for (size_t i = 0; i < dir->capacity; ++i) {
    set[dir->_[i]] = true;
  }
