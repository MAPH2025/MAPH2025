...
static constexpr uint8_t NUM_DIMMS = 2;
// static constexpr uint16_t PAGE_SIZE = 4 * 1024;  // 4kb
...
struct ViperConfig {
  ...
  uint8_t num_recovery_threads = 2;
...
struct alignas(PAGE_SIZE) ViperPage<std::string, std::string> {
  ...
  static constexpr size_t METADATA_SIZE = sizeof(version_lock) +
                                          sizeof(next_insert_pos) +
                                          sizeof(modified_percentage);
...
// 一键打印 DRAM / PMEM 空间开销 & 装载率
  void print_usage() const {
      // 2. 段内存估算
      const size_t segment_size = sizeof(cceh::Segment<K>);
      ...
      /* ============================================================
      * 改进方案：首先修改CCEH类，添加必要的接口
      * ============================================================ */
      
      // 修改后可以这样实现：
      
      std::cout << "===== Viper Memory Usage =====\n";
      
      // 1. 尝试获取CCEH的精确信息（如果接口存在）
      ...
      // 检查是否有相应的接口
      // 这里假设我们添加了这些方法
      directory_capacity = map_.get_directory_capacity();  // 需要添加这个方法
      segment_count = map_.get_segment_count();           // 需要添加这个方法
      global_depth = map_.get_global_depth();             // 需要添加这个方法
      
      // 如果接口不存在，使用估算
      if (directory_capacity == 0) {
          // 估算目录大小：哈希表容量可以提供线索
          // 每个段有kNumSlot个槽位
          const size_t total_slots_in_table = map_.Capacity();
          const size_t slots_per_segment = cceh::Segment<K>::kNumSlot;
          
          // 估算段数量（向上取整）
          segment_count = (total_slots_in_table + slots_per_segment - 1) / slots_per_segment;
          
          // 估算目录大小：找到不小于段数量的2的幂
          directory_capacity = 1;
          while (directory_capacity < segment_count) {
              directory_capacity <<= 1;
          }
          
          global_depth = std::log2(directory_capacity);
      }
      
      // 计算DRAM使用量
      const size_t directory_bytes = directory_capacity * sizeof(void*);
      const size_t segment_bytes = segment_count * sizeof(cceh::Segment<K>);
      const size_t cceh_dram_bytes = directory_bytes + segment_bytes;
      
      // Viper块指针数组
      const size_t vblock_ptr_bytes = v_blocks_.size() * sizeof(void*);
      
      // 其他DRAM数据结构
      // 包括：free_blocks队列、原子变量等
      const size_t other_dram_bytes = 
          free_blocks_.size_approx() * sizeof(block_size_t) +  // 队列中的元素
          sizeof(std::atomic<bool>) * 3 +  // is_resizing_, is_reclaiming_, deadlock_offset_lock_
          sizeof(std::atomic<size_t>) * 6; // current_size_, reclaimable_ops_, num_v_blocks_等
      
      const size_t dram_total_bytes = 
          cceh_dram_bytes + vblock_ptr_bytes + other_dram_bytes;
      ...
      * ============================================================ */
      ...
      // 已使用的PMem块（已写入至少一次）
      const size_t pmem_used_blocks = v_base_.v_metadata->num_used_blocks.load();
      const size_t pmem_used_bytes = pmem_used_blocks * BLOCK_SIZE;
      
      // 分配的PMem块（包括未使用的）
      const size_t pmem_allocated_blocks = v_base_.v_metadata->num_allocated_blocks;
      const size_t pmem_allocated_bytes = pmem_allocated_blocks * BLOCK_SIZE;
      
      // 映射的PMem总大小（可能大于分配的，因为包含元数据）
      const size_t pmem_mapped_bytes = v_base_.v_metadata->total_mapped_size;
      
      // PMem元数据大小
      const size_t pmem_metadata_bytes = 
          v_base_.v_metadata->block_offset +  // 初始元数据块
          (v_base_.v_mappings.size() > 1 ? 
          (v_base_.v_mappings.size() - 1) * v_base_.v_metadata->alloc_size : 0);
      ...
      * ============================================================ */
      ...
      // 固定大小条目的槽位利用率
      double pmem_slot_utilization = 0.0;
      double pmem_space_utilization = 0.0;
      
      if constexpr (!std::is_same_v<K, std::string>) {
          // 固定大小条目
          const size_t total_slots = pmem_used_blocks * 
                                    num_pages_per_block * 
                                    VPage::num_slots_per_page;
          
          if (total_slots > 0) {
              pmem_slot_utilization = 
                  static_cast<double>(current_size_.load()) / total_slots;
              
              // 空间利用率（考虑元数据开销）
              const size_t usable_data_bytes = current_size_.load() * (sizeof(K) + sizeof(V));
              const size_t total_storage_bytes = pmem_used_bytes;
              if (total_storage_bytes > 0) {
                  pmem_space_utilization = 
                      static_cast<double>(usable_data_bytes) / total_storage_bytes;
              }
          }
      } else {
          // 变长字符串条目 - 只能提供估算
          // 每个块的数据容量
          const size_t data_capacity_per_block = BLOCK_SIZE - sizeof(VPage);
          const size_t total_data_capacity = pmem_used_blocks * data_capacity_per_block;
          
          // 我们不知道实际数据大小，只能给出估计
          // 可以使用修改百分比来估算使用率
          size_t total_modified_percentage = 0;
          size_t counted_blocks = 0;
          
          // 扫描部分块来估算
          const size_t sample_size = std::min(pmem_used_blocks, static_cast<size_t>(100));
          for (size_t i = 0; i < sample_size && i < v_blocks_.size(); ++i) {
              if (v_blocks_[i] && !v_blocks_[i]->is_unused()) {
                  for (const auto& page : v_blocks_[i]->v_pages) {
                      total_modified_percentage += page.modified_percentage;
                      counted_blocks++;
                  }
              }
          }
          ...
          if (counted_blocks > 0) {
              pmem_space_utilization = 
                  static_cast<double>(total_modified_percentage) / (counted_blocks * 100);
          }
      }
      
      // CCEH装载率（哈希表）
      const size_t cceh_total_slots = map_.Capacity();
      const double cceh_load_factor = 
          cceh_total_slots > 0 ? 
          static_cast<double>(current_size_.load()) / cceh_total_slots : 0.0;
      ...
      * ============================================================ */
      ...
      std::cout << "===== Viper Detailed Memory Usage =====\n";
      
      // DRAM部分
      std::cout << "\nDRAM Usage:\n";
      std::cout << "  CCEH Index:\n";
      std::cout << "    Directory (" << directory_capacity << " entries) : " 
                << directory_bytes / MB << " MB\n";
      std::cout << "    Segments (" << segment_count << " segments)   : " 
                << segment_bytes / MB << " MB\n";
      std::cout << "    CCEH Total                          : " 
                << cceh_dram_bytes / MB << " MB\n";
      std::cout << "  Viper Structures:\n";
      std::cout << "    Block pointers (" << v_blocks_.size() << " ptrs) : " 
                << vblock_ptr_bytes / MB << " MB\n";
      std::cout << "    Other structures                  : " 
                << other_dram_bytes / MB << " MB\n";
      std::cout << "  DRAM Total                          : " 
                << dram_total_bytes / MB << " MB\n";
      
      // PMem部分
      std::cout << "\nPMem Usage:\n";
      std::cout << "  Used blocks     (" << pmem_used_blocks << " blocks) : " 
                << pmem_used_bytes / GB << " GB\n";
      std::cout << "  Allocated blocks (" << pmem_allocated_blocks << " blocks) : " 
                << pmem_allocated_bytes / GB << " GB\n";
      std::cout << "  Mapped size                        : " 
                << pmem_mapped_bytes / GB << " GB\n";
      std::cout << "  Metadata overhead                  : " 
                << pmem_metadata_bytes / MB << " MB\n";
      
      // 利用率指标
      std::cout << "\nUtilization Metrics:\n";
      std::cout << "  Current items stored          : " << current_size_.load() 
      <<"  total slots     : "<<cceh_total_slots<<"\n";
      
      if constexpr (!std::is_same_v<K, std::string>) {
          std::cout << "  PMem slot utilization       : " 
                    << pmem_slot_utilization * 100.0 << " %\n";
      }
      
      std::cout << "  PMem space utilization       : " 
                << pmem_space_utilization * 100.0 << " %\n";
      std::cout << "  CCEH load factor             : " 
                << cceh_load_factor * 100.0 << " %\n";
      
      // 其他统计
      std::cout << "\nOther Statistics:\n";
      std::cout << "  Free blocks in pool          : " 
                << free_blocks_.size_approx() << "\n";
      std::cout << "  Active clients               : " 
                << num_active_clients_.load() << "\n";
      std::cout << "  Global depth (estimated)     : " 
                << global_depth << "\n";
...
ViperInitData init_file_pool(const std::string& pool_dir, uint64_t pool_size,
  ...
  if (is_new_pool && std::filesystem::exists(pool_dir) &&
    ...
    throw std::runtime_error(
        "Cannot create new database in non-empty directory");
  }
...
void Viper<K, V>::trigger_resize() {
  ...
  // Only one thread can ever get here.
  resize_thread_ = std::make_unique<std::thread>([this] {
    ...
    const double resize_time_us =
        std::chrono::duration_cast<std::chrono::duration<double, std::micro>>(
            t_end - t_start)
            .count();

    printf("Resize time: %.2f us", resize_time_us);
  ...
  //resize_thread_->detach();
  if (resize_thread_ && resize_thread_->joinable()) {
    resize_thread_->join();
  }
}
...
bool Viper<std::string, std::string>::Client::put(const std::string& key,
  ...
  if (offset_in_page + meta_size + entry_length > v_page_size) {
    ...
    if (offset_in_page + meta_size + entry.key_size <= v_page_size &&
      ...
      v_page_->next_insert_pos =
          insert_pos + meta_size + value_entry.value_size;
      current_v_page = v_page_;
      is_inserted = true;
    } else {
      // Both key and value don't fit. Add both to new page. 0 sizes indicate
      // that both are on next page.
...
inline void Viper<K, V>::Client::invalidate_record(
    VPage* v_page, const data_offset_size_t data_offset) {
  if constexpr (std::is_same_v<K, std::string>) {
...
void Viper<std::string, std::string>::compact(Client& client,
  ...
  bool is_last_page = reinterpret_cast<const void*>(next_insert_pos) !=
                      reinterpret_cast<const void*>(v_page);
  ...
  while (true) {
    ...
    if (end_of_page || (var_entry.key_size == 0 && var_entry.value_size == 0)) {
      ...
      is_last_page = reinterpret_cast<const void*>(next_insert_pos) !=
                     reinterpret_cast<const void*>(v_page);
      continue;
    }
...
void Viper<K, V>::reclaim() {
  ...
  const size_t free_threshold =
      v_config_.reclaim_free_percentage * num_slots_per_block;
...
void Viper<std::string, std::string>::reclaim() {
  ...
  for (block_size_t block_num = 0; block_num < max_block; ++block_num) {
    ...
    for (const VPage& v_page : v_block->v_pages) {
      modified_percentage +=
          ((double)v_page.modified_percentage / num_pages_per_block) / 100;
