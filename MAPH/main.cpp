//g++ main.cpp -o main_pmem -lpmem -I. -O3 -std=c++17
#include <iostream>
#include <fstream>
#include <chrono>
#include <vector>
        ...
        if (found != std::string::npos) {
            ...
            if (userEnd != std::string::npos) {
                ...
                try {
                    ...
                    char val[VAL_LEN];
                    memset(val,VAL_LEN,1);
                    if (op.type == INSERT || op.type == UPDATE) {
                        //op.entry.val = *(uint64_t*)(val);
                        memcpy(op.entry.val,val,VAL_LEN);
                    }
...
double test_latency_cuckoo(TestOp op,
                           int total_ops = 2000000,
                           int neg_reads = 1000000) {
  CuckooHashTable table((TEST_SLOTS / (2 * BUCKET_SIZE)), 5);
  ...
  if (op == TestOp::INSERT) {
    for (int i = 0; i < total_ops; ++i) {
      if (!table.insert(operations[i].entry)) {
        printf("insert fail i:%d\n", i);
        break;
      }
      ops++;
  ...
  /* ---------------- READ / UPDATE / DELETE / READ_NEGATIVE ---------------- */
    ...
    if (op == TestOp::READ) {
      for (int i = 0; i < total_ops; ++i) {
        if (!table.query(run_ops[i].entry.key, result)) {
          printf("read fail i:%d key:%ld\n",
                 i, run_ops[i].entry.key);
          break;
        }
        ops++;
      }
    ...
    /* ---------------- READ NEGATIVE ---------------- */
      ...
      // 构造一个“必然不存在”的 key
      KEY_TYPE neg_key = operations[0].entry.key ^ (1ULL << 63);

      for (int i = 0; i < neg_reads; ++i) {
        table.query(neg_key, result);  // 不关心返回值
        ops++;
      }
    }
    ...
    /* ---------------- UPDATE ---------------- */
      ...
      for (int i = 0; i < total_ops; ++i) {
        if (!table.update(run_ops[i].entry)) {
    ...
    /* ---------------- DELETE ---------------- */
      ...
      for (int i = 0; i < total_ops; ++i) {
        if (!table.deletion(operations[i].entry.key)) {
  ...
  return std::chrono::duration<double>(t_end - t_begin).count()
         * 1e6 / ops;   // us/op
}
...
void multi_thread_op(CuckooHashTable &table, int begin, int end, chrono::duration<double>& time_cost){
    ...
    for(int i = begin; i < end; i++){
        ...
        switch (op.type) {
            ...
            case READ:
                table.query(op.entry.key, result);
                break;
...
int main(){
    ...
    const std::string load_path_skew = "../ycsb_data/temp_op/run_update_0.05_uniform.txt";
    read_ycsb_operations(load_path, operations);
    read_ycsb_operations(load_path_skew, run_ops);
    //test_recovery();
    // test_insert();
    //double ins = test_latency_cuckoo(TestOp::INSERT);
    //double rd  = test_latency_cuckoo(TestOp::READ);
    // double rd_n  = test_latency_cuckoo(TestOp::READ_NEGATIVE);
    //double upd = test_latency_cuckoo(TestOp::UPDATE);
    // double del = test_latency_cuckoo(TestOp::DELETE_OP);
