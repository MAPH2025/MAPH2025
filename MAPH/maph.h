...
#define PMEM_BUCKET_SIZE (BUCKET_SIZE + EXTRA_SLOTS)  
...
#define PMEM_BITMAP_BYTESIZE (((PMEM_BUCKET_SIZE + 7) / 8))
#define THREAD_NUM 2 //mark if use(1) or not use(2) multi thread 
#define RW_LOCK_CK
...
struct Bucket{  
    ...
    uint8_t pmem_cell_cnt[PMEM_BITMAP_BYTESIZE];
...
mutex lt_mutex;
chrono::duration<double> dram_lt(0);
chrono::duration<double> pmem_lt(0);
chrono::duration<double> lock_lt(0);
...
public:  
    ...
    inline SIG_TYPE calculate_fp(KEY_TYPE key){  
        SIG_TYPE fp = MurmurHash3_x86_32(&key, KEY_LEN, HASH_SEED_FP) & (~uint32_t(0)>>16);// % (1 << (SIG_LEN * 16));  
    ...
    inline void hash_key(KEY_TYPE key, SIG_TYPE &fp, int &idx) {
        uint32_t h = MurmurHash3_x86_32(&key, KEY_LEN, HASH_SEED_BUCKET);
        fp  = (SIG_TYPE)(h >> 16);
        idx = h % bucket_number;
    }
    ...
    inline bool kick(uint8_t table_idx,
                    int bucket_index,
                    uint8_t /*alt_index*/,
                    uint8_t cell_index) {
        constexpr int MAX_KICK = 10;
        KickNode path[MAX_KICK];

        path[0] = { table_idx, bucket_index, cell_index };

        for (int depth = 1; depth < MAX_KICK; ++depth) {
            KickNode &prev = path[depth - 1];

            SIG_TYPE fp = table[prev.table]
                            .bucket[prev.bucket]
                            .fp[prev.cell];

            int next_table = 1 - prev.table;
            int next_bucket = hash_alt(prev.bucket, prev.table, fp);

            int empty = bitmap_find_first_zero(
                table[next_table].bucket[next_bucket].cell_cnt,
                BITMAP_BYTESIZE
            );

            if (empty >= 0 && empty < BUCKET_SIZE) {
                path[depth] = { next_table, next_bucket, empty };

                for (int i = depth; i > 0; --i) {
                    KickNode &src = path[i - 1];
                    KickNode &dst = path[i];

                    table[dst.table].bucket[dst.bucket].fp[dst.cell] =
                        table[src.table].bucket[src.bucket].fp[src.cell];

                    table[dst.table].bucket[dst.bucket].pmem_idx[dst.cell] =
                        table[src.table].bucket[src.bucket].pmem_idx[src.cell];

                    bitmap_set(
                        table[dst.table].bucket[dst.bucket].cell_cnt,
                        dst.cell
                    );
                }

                bitmap_clear(
                    table[table_idx].bucket[bucket_index].cell_cnt,
                    cell_index
                );
    ...
    // inline bool kick(int table_idx, int bucket_index, int alt_index, int cell_index){  
    ...
    //         if(empty_index!=-1 && empty_index<BUCKET_SIZE){  
    ...
    inline bool insert(Entry& entry){  
        ...
        for(int i=0;i<BUCKET_SIZE;i++){  
            if((bitmap_test(table[TABLE1].bucket[idx1].cell_cnt,i) == 1 && fp_i == table[TABLE1].bucket[idx1].fp[i])){  
                ...
                if(pmem_table[pmem_table_idx].bucket[pmem_bucket_idx].pair[pmem_cell_idx].key == entry.key){
                    printf("Duplicate key:%ld, %ld\n",pmem_table[pmem_table_idx].bucket[pmem_bucket_idx].pair[pmem_cell_idx].key,entry.key);
        ...
        int pmem_bucket_size1 = bitmap_count_ones(table[TABLE1].bucket[idx1].pmem_cell_cnt,PMEM_BITMAP_BYTESIZE);
        int pmem_bucket_size2 = bitmap_count_ones(table[TABLE2].bucket[idx2].pmem_cell_cnt,PMEM_BITMAP_BYTESIZE);
        ...
        pmem_insert_cell = bitmap_find_first_zero(table[pmem_insert_table].bucket[pmem_insert_bucket].pmem_cell_cnt,PMEM_BITMAP_BYTESIZE);
        ...
        if(insert_flag==false){
            ...
            for(int i=0;i<BUCKET_SIZE;i++){
                if(kick(first_kick_table,idx,alt_idx,i)){
                    insert_flag = true;
                ...
                if(kick(1-first_kick_table,alt_idx,idx,i)){
                    insert_flag = true;
        ...
        bitmap_set(table[pmem_insert_table].bucket[pmem_insert_bucket].pmem_cell_cnt,pmem_insert_cell);
        ...
        for(int i=0;i<BUCKET_SIZE;++i){  
            ...
            if(bitmap_test(table[TABLE2].bucket[idx2].cell_cnt,i) == 1 && table[TABLE2].bucket[idx2].fp[i] == fp_i
             && pmem_table[pmem_table_idx].bucket[pmem_bucket].pair[pmem_cell].key == key){  
                ...
                bitmap_clear(table[pmem_table_idx].bucket[pmem_bucket].pmem_cell_cnt,pmem_cell);
    ...
    int cal_load_factor(){  
        ...
        for(int i=0;i<bucket_number;++i){  
            ...
            pmem_cnt1 += bitmap_count_ones(table[TABLE1].bucket[i].pmem_cell_cnt,PMEM_BITMAP_BYTESIZE);
            pmem_cnt2 += bitmap_count_ones(table[TABLE2].bucket[i].pmem_cell_cnt,PMEM_BITMAP_BYTESIZE); 
            if(bitmap_count_ones(table[TABLE1].bucket[i].pmem_cell_cnt,PMEM_BITMAP_BYTESIZE) > max_pmem_cnt){
                max_pmem_cnt = bitmap_count_ones(table[TABLE1].bucket[i].pmem_cell_cnt,PMEM_BITMAP_BYTESIZE);
            }
            if(bitmap_count_ones(table[TABLE2].bucket[i].pmem_cell_cnt,PMEM_BITMAP_BYTESIZE)>max_pmem_cnt){
                max_pmem_cnt = bitmap_count_ones(table[TABLE2].bucket[i].pmem_cell_cnt,PMEM_BITMAP_BYTESIZE);
            }
        }  
        ...
        double dram_use =
            2.0 * bucket_number * sizeof(Bucket) +
            2.0 * bucket_number * sizeof(shared_mutex) +
            sizeof(CuckooHashTable);

        double pmem_logical_use =
            2.0 * bucket_number * sizeof(PmemBucket);

        double pmem_mapped_use =
            (double)mapped_len;

        cout << "========== Memory Usage ==========\n";
        cout << "DRAM total        : "
            << dram_use / (1024*1024*1024) << " GB\n";
        cout << "PMem logical use  : "
            << pmem_logical_use / (1024*1024*1024) << " GB\n";
        cout << "PMem mapped use   : "
            << pmem_mapped_use / (1024*1024*1024) << " GB\n";
        cout << "==================================\n";
        // // 获取PMem访问统计信息  
