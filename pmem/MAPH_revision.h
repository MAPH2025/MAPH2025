...
public:  
    ...
    int last_insert_used_kick = 0;
    int last_insert_kick_path = 0;
    //-------------TIME-------------//
    ...
    void init_pmem(){
        pmem_read_count = 0;
        pmem_write_count = 0;
        size_t size = 2 * this->bucket_number * sizeof(PmemBucket);
    ...
    CuckooHashTable(int bucket_number, int max_kick_number){  
    ...
    void check_correct(){  
        for(int i=0;i<bucket_number;++i){  
            ...
            if(oc_c != bitmap_count_ones(table[TABLE1].bucket[i].pmem_cell_cnt,PMEM_BITMAP_BYTESIZE)){  
                printf("oc count missmatch table1 idx:%d, real: %d, bitmap: %d\n",i,oc_c, bitmap_count_ones(table[TABLE1].bucket[i].pmem_cell_cnt,PMEM_BITMAP_BYTESIZE));  
            }  
            //检查cell数量是否正确  
            if(counter_cnt != bitmap_count_ones(table[TABLE1].bucket[i].cell_cnt,BITMAP_BYTESIZE)){  
                printf("cell miss match table1 idx:%d, real: %d, cell: %d\n",i,counter_cnt, bitmap_count_ones(table[TABLE1].bucket[i].cell_cnt,PMEM_BITMAP_BYTESIZE));  
            }  
        }  
        ...
        for(int i=0;i<bucket_number;++i){  
            ...
            if(oc_c != bitmap_count_ones(table[TABLE2].bucket[i].pmem_cell_cnt,PMEM_BITMAP_BYTESIZE)){  
                printf("bytesize:%d\n",PMEM_BITMAP_BYTESIZE);
                printf("oc count missmatch table2 idx:%d ,real: %d, bitmap: %d\n",i,oc_c,bitmap_count_ones(table[TABLE2].bucket[i].pmem_cell_cnt,PMEM_BITMAP_BYTESIZE));  
            }  
            //检查cell数量是否正确  
    ...
    inline bool kick(int table_idx, int bucket_index, int alt_index, int cell_index, int* path_len){  
        ...
        if(find_empty == true){  
            ...
            if (path_len) *path_len = kick_times;
            return true;  
        }  
    ...
    inline bool insert(Entry& entry){  
        last_insert_used_kick = 0;
        last_insert_kick_path = 0;
...
///
        ...
        if(insert_flag==false){
            ...
            for(int i=0;i<BUCKET_SIZE;i++){
                int pl = 0;
                if(kick(first_kick_table,idx,alt_idx,i,&pl)){
                    insert_flag = true;
                    last_insert_used_kick = 1;
                    last_insert_kick_path = pl;
                ...
                int pl2 = 0;
                if(kick(1-first_kick_table,alt_idx,idx,i,&pl2)){
                    insert_flag = true;
                    last_insert_used_kick = 1;
                    last_insert_kick_path = pl2;
    ...
    inline bool recover_insert(Entry& entry){  
